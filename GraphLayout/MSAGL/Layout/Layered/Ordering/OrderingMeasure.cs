...
namespace Microsoft.Msagl.Layout.Layered {
    ...
    internal class OrderingMeasure {
        ...
        private double layerGroupDisbalance;
        private int[][] la;
        private int virtVertexStart;
        ...
        private void CalculateLayerGroupDisbalance() {
            for(int i = 0; i< this.la.Length;i++) {
                this.layerGroupDisbalance += this.LayerGroupDisbalance(this.la[i],this.optimalOriginalGroupSize[i],
                    this.optimalVirtualGroupSize[i]);
            }
        }

        private double LayerGroupDisbalance(int[] l, double origGroupOptSize, double virtGroupOptSize){
        ...
        private double LayerGroupDisbalanceWithVirtSeparators(int[] l, double origGroupOptSize) {
            ...
            for (int i = 0; i < l.Length; ) {
                ret += this.CurrentOrigGroupDelta(ref i, l, origGroupOptSize);
            }
        ...
        private double LayerGroupDisbalanceWithOrigSeparators(int[] l, double virtGroupOptSize) {
            double ret = 0;
            for (int i = 0; i < l.Length; ) {
                ret += this.CurrentVirtGroupDelta(ref i, l, virtGroupOptSize);
            }
