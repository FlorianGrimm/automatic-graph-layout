    ...
    public class SplineRouter : AlgorithmBase {
        ...
        */
        ...
        private void RouteMultiEdges(List<Edge[]> multiEdges, InteractiveEdgeRouter interactiveEdgeRouter, Set<Shape> parents) {
            var mer = new MultiEdgeRouter(
                multiEdges,
                interactiveEdgeRouter,
                parents.SelectMany(p => p.Children).Select(s => s.BoundaryCurve),
                 new BundlingSettings { InkImportance = 0.00001, EdgeSeparation = this.MultiEdgesSeparation },
                 this.MakeTransparentShapesOfEdgeGeometryAndGetTheShapes);
...
#if TEST_MSAGL
        [SuppressMessage("Microsoft.Performance", "CA1811:AvoidUncalledPrivateCode")]
        static internal void ShowVisGraph(
            VisibilityGraph tmpVisGraph,
            IEnumerable<Polyline> obstacles,
            IEnumerable<ICurve>? greenCurves = null,
            IEnumerable<ICurve>? redCurves = null) {
