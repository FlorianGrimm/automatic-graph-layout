...
namespace Microsoft.Msagl.Routing.Spline.Bundling {
    ...
    internal class CdtIntersections {
        ...
        internal bool BundleAvoidsObstacles(Station v, Station u, Point vPosition, Point uPosition, double upperBound,
            ...
            Set<Polyline> obstaclesToIgnore = this.metroGraphData.looseIntersections.ObstaclesToIgnoreForBundle(v, u);
            Dictionary<Polyline, Tuple<Point, Point>> closeObstacles = this.FindCloseObstaclesForBundle(u.CdtTriangle, uPosition, vPosition, obstaclesToIgnore, upperBound);
        ...
        /// </summary>
        private Dictionary<Polyline, Tuple<Point, Point>> FindCloseObstaclesForBundle(CdtTriangle startTriangle, Point start,
                                                                              Point end, Set<Polyline> obstaclesToIgnore,
                                                                              double upperBound) {
            var obstacles = new Dictionary<Polyline, Tuple<Point, Point>>();
            List<CdtTriangle> list;
            if (!this.ThreadLineSegmentThroughTriangles(startTriangle, start, end, obstaclesToIgnore, out list)) {
                return null;
            }
        ...
        /// returns false iff the edge overlap an obstacle
        /// </summary>
        private bool ThreadLineSegmentThroughTriangles(CdtTriangle currentTriangle, Point start, Point end, Set<Polyline> obstaclesToIgnore,
        ...
        private static PolylinePoint FindPolylinePoint(Polyline poly, Point point) {
        ...
        internal bool EdgeIsLegal(Station v, Station u, Point vPosition, Point uPosition) {
            List<CdtTriangle> list;
            Set<Polyline> obstaclesToIgnore = this.metroGraphData.looseIntersections.ObstaclesToIgnoreForBundle(v, u);
            return this.ThreadLineSegmentThroughTriangles(v.CdtTriangle, vPosition, uPosition, obstaclesToIgnore, out list);
        }
