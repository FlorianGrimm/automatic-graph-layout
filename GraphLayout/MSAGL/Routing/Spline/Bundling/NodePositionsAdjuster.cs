...
namespace Microsoft.Msagl.Routing.Spline.Bundling {
    ...
    public class NodePositionsAdjuster {
        ...
        private NodePositionsAdjuster(MetroGraphData metroGraphData, BundlingSettings bundlingSettings) {
...
//            HubDebugger.ShowHubs(metroGraphData, bundlingSettings);
        ...
        /// </summary>
        private bool GlueConflictingNodes() {
            ...
            RectangleNodeUtils.CrossRectangleNodes<Station, Point>(circlesHierarchy, circlesHierarchy,
                                                            (i, j) => this.TryToGlueNodes(i, j, gluingMap, gluedDomain));
            if (gluingMap.Count == 0) {
                return false;
            }

            for (int i = 0; i < this.metroGraphData.Edges.Length; i++) {
            ...
            SimulatedAnnealing.FixRouting(this.metroGraphData, this.bundlingSettings, affectedPoints);
            return true;
        }

        private RectangleNode<Station, Point> GetCirclesHierarchy() {
            foreach (var v in this.metroGraphData.VirtualNodes()) {
                v.Radius = this.GetCurrentHubRadius(v);
            }

            return RectangleNode<Station, Point>.CreateRectangleNodeOnEnumeration(from i in this.metroGraphData.VirtualNodes()
                                                                           let p = i.Position
        ...
        private double GetCurrentHubRadius(Station node) {
            if (node.IsRealNode) {
                ...
                double r = this.metroGraphData.looseIntersections.GetMinimalDistanceToObstacles(node, node.Position, idealR);
        ...
        private void TryToGlueNodes(Station i, Station j, Dictionary<Station, Station> gluingMap, Set<Station> gluedDomain) {
            Debug.Assert(i != j);
        ...
        private bool TryGlueOrdered(Station i, Station j, Set<Station> gluedDomain, Dictionary<Station, Station> gluingMap) {
            if (!gluingMap.ContainsKey(i) && !gluedDomain.Contains(i) && this.NodeGluingIsAllowed(i, j, gluingMap)) {
        ...
        private void Map(Station i, Station j, Set<Station> gluedDomain, Dictionary<Station, Station> gluingMap) {
            gluingMap[i] = j;
        ...
        /// </summary>
        private bool NodeGluingIsAllowed(Station i, Station j, Dictionary<Station, Station> gluingMap) {
            foreach (var adj in i.Neighbors) {
                var k = Glued(adj, gluingMap);
                //1. check that we can merge these stations (== no intersections)
                Set<Polyline> obstaclesToIgnore = this.metroGraphData.looseIntersections.ObstaclesToIgnoreForBundle(k, i);
                if (!this.metroGraphData.cdtIntersections.EdgeIsLegal(k, j, k.Position, j.Position, obstaclesToIgnore)) {
                    return false;
                }
            }

            //2. check that cost of the routing is reduced
            double delta = this.ComputeCostDeltaAfterNodeGluing(i, j, gluingMap);
            if (delta < 0) {
                return false;
            }
        ...
        private double ComputeCostDeltaAfterNodeGluing(Station i, Station j, Dictionary<Station, Station> gluingMap) {
            double d = (i.Position - j.Position).Length;
            ...
            double newInk = this.metroGraphData.Ink - (j.Position - i.Position).Length;
            foreach (var adj in i.Neighbors) {
                var k = Glued(adj, gluingMap);
                newInk -= (k.Position - i.Position).Length;
                newInk += (this.metroGraphData.RealEdgeCount(k, j) == 0 ? (k.Position - j.Position).Length : 0);
            }

            gain += CostCalculator.InkError(oldInk, newInk, this.bundlingSettings);

            //path lengths
            foreach (var metroInfo in this.metroGraphData.MetroNodeInfosOfNode(i)) {
        ...
        private void RegenerateEdge(Dictionary<Station, Station> gluingMap, int edgeIndex) {
            var poly = this.metroGraphData.Metrolines[edgeIndex].Polyline;
            if (!poly.Any(p => gluingMap.ContainsKey(this.metroGraphData.PointToStations[p]))) {
                return;
            }

            this.metroGraphData.Edges[edgeIndex].Curve =
                new Polyline(GluedPolyline(poly.Select(p => this.metroGraphData.PointToStations[p]).ToArray(), gluingMap));
        }

        private static IEnumerable<Point> GluedPolyline(Station[] metroline, Dictionary<Station, Station> gluedMap) {
            int i;
        ...
        private static Station Glued(Station i, Dictionary<Station, Station> gluedMap) {
            Station j;
        ...
        private bool UnglueEdgesFromBundleToSaveInk(bool alwaysExecuteSA) {
            ...
            foreach (var metroline in this.metroGraphData.Metrolines) {
                var obstaclesAllowedToIntersect =
                    this.metroGraphData.PointToStations[metroline.Polyline.Start].EnterableLoosePolylines*
                    this.metroGraphData.PointToStations[metroline.Polyline.End].EnterableLoosePolylines;
            ...
            if (progress) //TimeMeasurer.DebugOutput("unbundling");
{
            ...
            if (alwaysExecuteSA || progress) {
                SimulatedAnnealing.FixRouting(this.metroGraphData, this.bundlingSettings, alwaysExecuteSA ? null : affectedPoints);
            }

            return progress;
        }

        private bool TrySeparateOnPolyline(Metroline metroline, Dictionary<PointPair, Set<Metroline>> segsToPolylines,
        ...
        private bool TryShortcutPolypoint(PolylinePoint pp, Dictionary<PointPair, Set<Metroline>> segsToPolylines, HashSet<Point> affectedPoints, Set<Polyline> obstaclesAllowedToIntersect) {            
        ...
        /// </summary>
        private bool SeparationShortcutAllowed(PolylinePoint pp, Dictionary<PointPair, Set<Metroline>> segsToPolylines, Set<Polyline> obstaclesAllowedToIntersect) {
            var a = pp.Point;
            var b = pp.Next.Point;
            var c = pp.Next.Next.Point;
            var aStation = this.metroGraphData.PointToStations[a];
            var bStation = this.metroGraphData.PointToStations[b];
            var cStation = this.metroGraphData.PointToStations[c];
            //1. intersections
            if (!this.metroGraphData.cdtIntersections.EdgeIsLegal(aStation, cStation,
                                                             a, c,
        ...
        private double GetInkgain(PolylinePoint pp, Dictionary<PointPair, Set<Metroline>> segsToPolylines, Point a, Point b, Point c) {
            Set<Metroline> abPolylines, bcPolylines, abcPolylines;
            this.FindPolylines(pp, segsToPolylines, out abPolylines, out bcPolylines, out abcPolylines);
            double gain = 0;
            //ink
            double oldInk = this.ink;
            double newInk = this.ink;
            ...
            foreach (var metroline in abcPolylines) {
                double oldLength = this.polylineLength[metroline];
                double newLength = this.polylineLength[metroline];
                newLength -= ab + bc - ac;
            ...
            //radii
            double nowR = this.GetCurrentHubRadius(this.metroGraphData.PointToStations[a]);
            double widthABC = this.metroGraphData.GetWidth(abcPolylines, this.bundlingSettings.EdgeSeparation);
            double widthABD = this.metroGraphData.GetWidth(abPolylines - abcPolylines, this.bundlingSettings.EdgeSeparation);
            double idealR = HubRadiiCalculator.GetMinRadiusForTwoAdjacentBundles(nowR, a, c, b, widthABC, widthABD, this.metroGraphData, this.bundlingSettings);
            if (idealR > nowR) {
                gain -= CostCalculator.RError(idealR, nowR, this.bundlingSettings);
            }

            //check opposite side
            nowR = this.GetCurrentHubRadius(this.metroGraphData.PointToStations[c]);
            double widthCBD = this.metroGraphData.GetWidth(bcPolylines - abcPolylines, this.bundlingSettings.EdgeSeparation);
            idealR = HubRadiiCalculator.GetMinRadiusForTwoAdjacentBundles(nowR, c, b, a, widthCBD, widthABC, this.metroGraphData, this.bundlingSettings);
            if (idealR > nowR) {
                gain -= CostCalculator.RError(idealR, nowR, this.bundlingSettings);
            }
        ...
        private void RemoveShortcuttedPolypoint(PolylinePoint pp, Dictionary<PointPair, Set<Metroline>> segsToPolylines) {
            var a = pp.Point;
            var b = pp.Next.Point;
            var c = pp.Next.Next.Point;
            ...
            //fixing edge lengths
            foreach (var metroline in abcPolylines) {
                this.polylineLength[metroline] -= ab + bc - ac;
            }

            //fixing polylines
            foreach (var metroline in abcPolylines) {
                this.RemovePolypoint(metroline.Polyline.PolylinePoints.First(p => p.Point == b));
        ...
        private void FindPolylines(PolylinePoint pp, Dictionary<PointPair, Set<Metroline>> segsToPolylines,
        ...
        private void RemovePolypoint(PolylinePoint p) {
        ...
        /// </summary>
        private bool GlueCollinearNeighbors(int step) {
            HashSet<Point> affectedPoints = new HashSet<Point>();
            bool progress = false;
            foreach (var node in this.metroGraphData.Stations) {
                if (this.GlueCollinearNeighbors(node, affectedPoints, step)) {
                    progress = true;
                }
            }
        ...
        private bool GlueCollinearNeighbors(Station node, HashSet<Point> affectedPoints, int step) {
        ...
        private void TryToGlueEdges(Station node, Station a, Station b, Dictionary<Tuple<Station, Station>, Point> gluedEdges, int step) {
            Debug.Assert(a != b);
            ...
            if (angle < this.bundlingSettings.AngleThreshold) {
                ...
                if (la < lb) {
                    if (this.EdgeGluingIsAllowed(node, a, b)) {
                        this.AddEdgeToGlue(node, b, a, a.Position, gluedEdges);
                        return;
                    }
                }
                else {
                    if (this.EdgeGluingIsAllowed(node, b, a)) {
                ...
                //TODO: need this???
                if (step < 5 && ratio > 0.5) {
                    Point newPosition = this.ConstructGluingPoint(node, a, b);
                    if (this.EdgeGluingIsAllowed(node, a, b, newPosition)) {
        ...
        private Point ConstructGluingPoint(Station node, Station a, Station b) {
            //temp
        ...
        private bool EdgeGluingIsAllowed(Station node, Station a, Station b) {
            ...
            var crossingsOfEdgeNodeA = InteractiveEdgeRouter.IntersectionsOfLineAndRectangleNodeOverPolyline(new LineSegment(node.Position, a.Position), this.metroGraphData.LooseTree);
            if (crossingsOfEdgeNodeA.Exists(ii => !enterableForEdgeNodeB.Contains(ii.Segment1))) {
                return false;
            }

            var crossingsOfEdgeab = InteractiveEdgeRouter.IntersectionsOfLineAndRectangleNodeOverPolyline(new LineSegment(a.Position, b.Position), this.metroGraphData.LooseTree);
            if (crossingsOfEdgeab.Exists(ii => !enterableForEdgeNodeB.Contains(ii.Segment1))) {
                return false;
            }

            //2. check cost
            double delta = this.ComputeCostDeltaAfterEdgeGluing(node, a, b, a.Position);
            if (delta < 0) {
                return false;
            }
        ...
        private bool EdgeGluingIsAllowed(Station node, Station a, Station b, Point gluingPoint) {
            //0. can't pass through real nodes
            if (!this.metroGraphData.looseIntersections.HubAvoidsObstacles(gluingPoint, 0, a.EnterableLoosePolylines * b.EnterableLoosePolylines)) {
                return false;
            }
            ...
            //2. check cost
            double delta = this.ComputeCostDeltaAfterEdgeGluing(node, a, b, gluingPoint);
            if (delta < 0) {
                return false;
            }
        ...
        private double ComputeCostDeltaAfterEdgeGluing(Station node, Station a, Station b, Point newp) {
            double gain = 0;

            //ink
            double oldInk = this.metroGraphData.Ink;
            double newInk = this.metroGraphData.Ink - (node.Position - b.Position).Length - (node.Position - a.Position).Length +
                (node.Position - newp).Length + (newp - a.Position).Length + (newp - b.Position).Length;
            gain += CostCalculator.InkError(oldInk, newInk, this.bundlingSettings);

            //path lengths
            foreach (var metroline in this.metroGraphData.GetIjInfo(node, b).Metrolines) {
            ...
            foreach (var metroline in this.metroGraphData.GetIjInfo(node, a).Metrolines) {
                double oldLength = metroline.Length;
            ...
            double nowR = this.GetCurrentHubRadius(node);
            double idealR = HubRadiiCalculator.GetMinRadiusForTwoAdjacentBundles(nowR, node, node.Position, a, b, this.metroGraphData, this.bundlingSettings);
            ...
            if (id2 > (node.Position - newp).Length && !node.IsRealNode) {
                gain -= CostCalculator.RError(id2, (node.Position - newp).Length, this.bundlingSettings);
            }

            return gain;
        }

        private void AddEdgeToGlue(Station node, Station b, Station a, Point newp, Dictionary<Tuple<Station, Station>, Point> gluedEdges) {
            //same edge in the reverse direction      
            if (gluedEdges.ContainsKey(new Tuple<Station, Station>(a, node))) {
                return;
            }

            if (gluedEdges.ContainsKey(new Tuple<Station, Station>(b, node))) {
                return;
            }

            if (gluedEdges.ContainsKey(new Tuple<Station, Station>(node, a))) {
                return;
            }

            if (gluedEdges.ContainsKey(new Tuple<Station, Station>(node, b))) {
                return;
            }
        ...
        private void GlueEdge(KeyValuePair<Tuple<Station, Station>, Point> keyValuePair) {
        ...
        private void SplitPolylinePoint(PolylinePoint node, Point pointToInsert) {
        ...
        /// </summary>
        private bool RelaxConstrainedEdges() {
            HashSet<Point> affectedPoints = new HashSet<Point>();
            bool progress = false;
            foreach (var edge in this.metroGraphData.VirtualEdges()) {
                if (this.RelaxConstrainedEdge(edge.Item1, edge.Item2, affectedPoints)) {
                    progress = true;
                }
            }
        ...
        private bool RelaxConstrainedEdge(Station a, Station b, HashSet<Point> affectedPoints) {
            //find conflicting obstacles
            double idealWidth = this.metroGraphData.GetWidth(a, b, this.bundlingSettings.EdgeSeparation);
            List<Tuple<Point, Point>> closestPoints;
            /*bool res =*/
            this.metroGraphData.cdtIntersections.BundleAvoidsObstacles(a, b, a.Position, b.Position, 0.99 * idealWidth / 2.0, out closestPoints); 
           // Debug.Assert(res); //todo still unsolved
            ...
            if (closestPoints.Count > 0) {
                ...
                if (!this.metroGraphData.looseIntersections.HubAvoidsObstacles(bestPoint, 0, a.EnterableLoosePolylines * b.EnterableLoosePolylines)) {
                    return false;
                }
                ...
                foreach (var metroline in this.metroGraphData.GetIjInfo(a, b).Metrolines) {
        ...
        /// switch flips
        /// </summary>
        private bool RemoveDoublePathCrossings() {
            bool progress = new PathFixer(this.metroGraphData, this.metroGraphData.PointIsAcceptableForEdge).Run();
