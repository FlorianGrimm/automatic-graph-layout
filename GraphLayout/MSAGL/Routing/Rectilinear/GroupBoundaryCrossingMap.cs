...
namespace Microsoft.Msagl.Routing.Rectilinear {
    ...
    internal class GroupBoundaryCrossingMap {
        ...
        private readonly Dictionary<Point, List<GroupBoundaryCrossing>> pointCrossingMap = new Dictionary<Point, List<GroupBoundaryCrossing>>();
        ...
        internal GroupBoundaryCrossing AddIntersection(Point intersection, Obstacle group, Direction dirToInside) {
            ...
            if (!this.pointCrossingMap.TryGetValue(intersection, out crossings)) {
                crossings = new List<GroupBoundaryCrossing>();
                this.pointCrossingMap[intersection] = crossings;
            }
        ...
        internal PointAndCrossingsList GetOrderedListBetween(Point start, Point end) {
            if (0 == this.pointCrossingMap.Count) {
                return null;
            }
            ...
            foreach (var intersection in this.pointCrossingMap.Keys) {
            ...
            var numCrossings = this.pointList.Count;
            for (int ii = 0; ii < numCrossings; ++ii) {
                Point intersect = this.pointList[ii];
                pointAndCrossingList.Add(intersect, this.pointCrossingMap[intersect]);
            }
        ...
        public override string ToString() {
            return string.Format(System.Globalization.CultureInfo.InvariantCulture, "{0}", this.pointCrossingMap.Count);
        }
    }
}
